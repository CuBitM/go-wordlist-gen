...
// ══════════════════════════════════════════════════════════════════
// BATCH CHANNEL – posíláme [][]byte místo string
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
// WORD CACHE – předvýpočet všech variant jednou
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
// NORMALIZACE DIAKRITIKY – vlastní tabulka (bez externích deps)
// ══════════════════════════════════════════════════════════════════
...
// emit přijme []byte a přidá ho do aktuálního batche
...
// join spojí kusy do builderu a zavolá emitStr – recykluje builder
...
func main() {
	...
	// Předvýpočet WordCache pro každé slovo
	...
	// Writer goroutine – zpracovává celé batche najednou
	go func() {
		defer writerWg.Done()
	...
	// ── Priorita 2 – keyboard walks
	runSeq(func(em *Emitter) { genPriority2(caches, lang, em) })
	// ── Priorita 3 – dvě slova (paralelně)
	var p3 []func(*Emitter)
	for i := range caches {
		i := i
	...
	runParallel(p3)
	// ── Priorita 4 – datum vzory (paralelně)
	var p4 []func(*Emitter)
	for i := range caches {
		i := i
	...
	runParallel(p4)
	// ── Priorita 5 – trojkombinace + partial leet
